...
class ScorecardGenerator(object):
    ...
    def __run_queries(self):
        ...
        for r in self.__requests:
            ...
            self.__all_cybex_orgs_with_descendants += self.__cyhy_db.RequestDoc.get_all_descendants(
                r["_id"]
            )
        ...
        for r in self.__requests:
            ...
            for trustymail_result_set in [
                ...
                for trustymail_result in self.__results[
                    ...
                    if trustymail_result["_id"] == score["owner"]:
            ...
            for https_scan_result in self.__results["https-scan"]:
                # Found info for the current org
                if https_scan_result["_id"] == score["owner"]:
            ...
            for sslyze_scan_result in self.__results["sslyze-scan"]:
                # Found info for the current org
                if sslyze_scan_result["_id"] == score["owner"]:
            ...
            # Found info for the current org
            if self.__results["cert-scan"].get(score["owner"]):
            ...
            for t in self.__tallies:
                ...
                if t["_id"] == r["_id"]:
                    ...
                    for vuln_scan_result in self.__results["vuln-scan"][
                        ...
                        # Found info for the current org
                        if vuln_scan_result["_id"]["owner"] == score["owner"]:
                    ...
                    # Hack for CYHY-441; exclude criticals less than 7 days old
                    # from the open_criticals total
    ...
    def generate_cybex_scorecard(self):
        ...
        if self.__anonymize:
            ...
            # Don't log creation of anonymous scorecards to the DB
            self.__log_scorecard_to_db = False
    ...
    def __generate_mustache_json(self, filename):
        ...
        if self.__log_scorecard_to_db:
            ...
            # If scorecard_oid is None, it will not be included in the PDF metadata
